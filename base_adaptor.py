...
class BaseAdaptor():
    ...
    def load_h36_cluster_res(self, ):
        ...
        self.h36m_base_features = np.concatenate(
            joblib.load('data/retrieval_res/h36m_feats_random_sample_center_10_10.pt'), axis=0)  # (31176,2048)
        # 按中心排序的特征

    def retrieval(self, feature):  # 在源域找6张？最接近的作为3D关节点监督
    ...
    def set_model_optim(self, ):
        ...
        """
        checkpoint = torch.load(self.options.model_file)  # data/basemodel.pt--119547037146038801333356
        model = vibe(config.SMPL_MEAN_PARAMS)
        if self.options.use_boa:
            self.model = l2l.algorithms.MAML(model, lr=self.options.fastlr, first_order=True).to(self.device)  # HMR
    ...
    def set_criterion(self, ):
        self.gmm_f = MaxMixturePrior(prior_folder='data/spin_data', num_gaussians=8, dtype=torch.float32).to(
            self.device)
    ...
    def upper_level_adaptation(self, image, gt_keypoints_2d, h36m_batch, learner=None):
        ...
        if self.options.use_frame_losses_upper:
            ...
            s2dloss = (F.mse_loss(pred_s2d[:, 25:], gt_keypoints_2d[:, 25:, :-1], reduction='none') * conf).mean()
            # shape prior constraint
            shape_prior = self.cal_shape_prior(pred_shape)  # 12.9445
            # pose prior constraint
            pose_prior = self.cal_pose_prior(pred_rotmat, pred_shape)  # 136.9978
            loss = s2dloss * self.options.s2dloss_weight + \
                   shape_prior * self.options.shape_prior_weight + \
                   pose_prior * self.options.pose_prior_weight
            # 0.3255
        ...
        if self.options.use_temporal_losses_upper:  # 1
            if self.options.use_meanteacher:
                teacherloss = self.cal_teacher_loss(image, pred_rotmat, pred_shape, pred_s2d, pred_s3d)  # 0.0023
                if self.options.use_frame_losses_upper:
                    loss += teacherloss * self.options.teacherloss_weight
                else:
                    loss = teacherloss * self.options.teacherloss_weight
    ...
    def adapt_on_labeled_data(self, model, batch, prefix='ll'):
        ...
        gt_rotmat = batch_rodrigues(gt_pose.view(-1, 3)).view(-1, 24, 3, 3)  # 转换！
