...
class Adaptor(BaseAdaptor):
    ...
    def excute(self, ):
        ...
        pve_all = []
        self.mpjpe_statistics, self.pampjpe_statistics = [[] for i in range(len(self.dataloader))], [[] for i in range(
            len(self.dataloader))]  # 35515个[]
        self.mpjpe_all_lower, self.pampjpe_all_lower = [[] for i in range(self.options.inner_step)], [[] for i in range(
        ...
        for step, batch in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            self.global_step = step  # 索引
            self.fit_losses = {}
            batch = {k: v.to(self.device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
            # 如果v是向量就传到GPU上，否则还是v
            # Step1: adaptation
            self.model.eval()  # 预测阶段，自动把BN和DropOut固定住。也会加上torch.no_grad()来关闭梯度的计算。
            ...
            if self.options.dynamic_boa:  # 重复优化，适应关键帧
                ...
                while 1 - feat_12 > self.options.cos_sim_threshold:
                    ...
                    if self.optimized_step > self.options.optim_steps:
                        ...
                        break
                    upper_level_loss, adapted_features = self.upper_level_adaptation(image, gt_keypoints_2d, h36m_batch,
                                                                                     self.model)
    ...
    def inference(self, batch, model, need_feature=False):
        ...
        pred_keypoints_3d = torch.matmul(J_regressor_batch, pred_vertices)  # 处理维度不同的tensor结构进行相乘
        ...
        # compute PVE
        smpl_out = self.smpl_neutral(betas=gt_betas, body_pose=gt_pose[:, 3:], global_orient=gt_pose[:, :3],
                                     pose2rot=True)
